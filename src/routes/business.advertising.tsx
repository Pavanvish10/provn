...
export const Route = createFileRoute("/business/advertising")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Promote open roles to relevant verified candidates on Provn." },
      { property: "og:title", content: "Job Advertising · Provn Business" },
      { property: "og:description", content: "Reach the right verified candidates for your open roles." },
    ],
  }),
    ...
    <AppShell>
      ...
      <header className="mb-8">
        ...
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
          Boost open roles across the Provn feed, targeted to candidates with matching verified skills.
        </p>
      </header>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Stat icon={<Eye className="h-4 w-4 text-brand" />} label="Impressions this week" value="—" />
        <Stat icon={<MousePointerClick className="h-4 w-4 text-brand" />} label="Clicks" value="—" />
      ...
      <div className="mt-6 rounded-2xl border border-dashed border-border bg-card p-10 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Post a job first, then promote it to the right verified candidates.</p>
        <Button asChild className="mt-4"><Link to="/business/hiring">Post a job</Link></Button>
      </div>
    </AppShell>
  );
}
