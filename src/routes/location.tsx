...
const CITIES = [
  "Bengaluru, India","Hyderabad, India","Mumbai, India","Delhi NCR, India","Pune, India","Chennai, India",
  "Kolkata, India","Ahmedabad, India","Remote · India","London, UK","New York, USA","San Francisco, USA",
...
export const Route = createFileRoute("/location")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Tell Provn where you are so we can tailor roles and roadmaps." },
      ...
      <div className="mx-auto grid min-h-[calc(100vh-6rem)] max-w-7xl grid-cols-1 gap-16 px-6 pb-16 lg:grid-cols-2 lg:items-center">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-brand-soft via-background to-background p-10">
            ...
            <div className="relative">
              ...
              <h2 className="mt-3 font-display text-4xl leading-tight tracking-tight sm:text-5xl">
                Local roles.<br /> Global standards.
              </h2>
              <p className="mt-4 max-w-sm text-sm text-muted-foreground">
                We use your city to match you with hybrid, on‑site, and remote roles nearby — and
                to calibrate salary bands honestly.
              </p>
              <ul className="mt-8 space-y-3 text-sm">
                {["Curated roles within commute range","Cost‑of‑living aware salary ranges","Chapters &amp; meetups near you"].map((t) => (
                  <li key={t} className="flex items-start gap-3">
                    <span className="mt-1 h-1.5 w-1.5 rounded-full bg-brand" />
                    <span className="text-muted-foreground" dangerouslySetInnerHTML={{ __html: t }} />
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="order-1 lg:order-2">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-2.5 py-1 text-xs text-muted-foreground">
            <span className="h-1.5 w-1.5 rounded-full bg-brand" /> Step 2 of 4 · Location
          </span>
          <h1 className="mt-5 font-display text-5xl leading-tight tracking-tight sm:text-6xl">Where are you based?</h1>
          ...
          <div className="mt-3 max-h-72 overflow-y-auto rounded-xl border border-border bg-card">
            ...
            {filtered.length === 0 && (
              <div className="px-4 py-6 text-sm text-muted-foreground">No matches — try another name.</div>
            )}
          </div>

          <div className="mt-6 flex gap-3">
            <Button variant="ghost" onClick={() => nav({ to: "/value-prop" })}>Back</Button>
            <Button size="lg" disabled={!selected} onClick={submit} className="ml-auto">
