...
const getPlatformStatsFn = createServerFn({ method: "GET" }).handler(async () => {
  const supabase = getSupabaseServerClient();
  const [{ count: verifiedCount }, { count: hiredCount }] = await Promise.all([
  ...
  return { verified: verifiedCount ?? 0, hired: hiredCount ?? 0 };
});

export const Route = createFileRoute("/value-prop")({
  beforeLoad: requireAuth,
  loader: () => getPlatformStatsFn(),
  head: () => ({
    meta: [
      { title: "Prove Your Skills, Not Just Résumé · Provn" },
      {
        name: "description",
        content: "See how Provn helps you learn, verify, prove, and get hired.",
      },
...
const FEATURES = [
  {
    ...
    copy: "Get a brutally honest ATS score and rewrite suggestions in under 30 seconds.",
  },
  {
    icon: Briefcase,
    title: "Get Hired",
    copy: "Apply once verified — recruiters see proof of skill, not just claims.",
  },
  {
    icon: Map,
    title: "AI Roadmap",
    copy: "A step‑by‑step plan tuned to your goal role, updated as you progress.",
  },
  {
    icon: Mic,
    title: "Mock Interview",
    copy: "Voice‑driven simulations that grade communication, structure, and depth.",
  },
  {
    icon: Code2,
    title: "Coding Challenges",
    copy: "Daily problems that build a public streak recruiters actually look at.",
  },
  {
    icon: Sparkles,
    title: "Personalized Career Path",
    copy: "One journey — learn, verify, prove, apply — no tab‑hopping.",
  },
];
      ...
      <div className="mx-auto grid min-h-[calc(100vh-6rem)] max-w-7xl grid-cols-1 gap-16 px-6 pb-16 lg:grid-cols-2 lg:items-center">
        <div>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-2.5 py-1 text-xs text-muted-foreground">
            <span className="h-1.5 w-1.5 rounded-full bg-brand" /> Step 1 of 6 · Welcome
          </span>
          ...
          <p className="mt-6 max-w-md text-base text-muted-foreground">
            Provn stitches learning, verification, projects, and hiring into one continuous journey
            — so what you can do speaks louder than what you claim.
          </p>
          ...
          {(stats.verified > 0 || stats.hired > 0) && (
            <div className="mt-10 flex items-center gap-6 text-xs text-muted-foreground">
              <div>
                <span className="font-display text-2xl text-foreground">{stats.verified}</span>{" "}
                skills verified
              </div>
              <div className="h-6 w-px bg-border" />
              <div>
                <span className="font-display text-2xl text-foreground">{stats.hired}</span> hired
                via Provn
              </div>
            </div>
          )}
